...
public class FindScorecardNamesQuery extends AbstractFindQuery implements NamedQuery {
    ...
    public Query toQuery(Set<ValueIndexTerm> terms) {
        ...
        ValueSharedPartIndexTerm sharedPartIndexTerm = null;
        ValueModuleRootPathIndexTerm moduleRootPathIndexTerm = null;
        ValuePackageNameIndexTerm packageNameIndexTerm = null;

        for (ValueIndexTerm term : terms) {
            if (term instanceof ValueSharedPartIndexTerm) {
                sharedPartIndexTerm = (ValueSharedPartIndexTerm) term;
            } else if (term instanceof ValueModuleRootPathIndexTerm) {
                moduleRootPathIndexTerm = (ValueModuleRootPathIndexTerm) term;
            } else if (term instanceof ValuePackageNameIndexTerm) {
                packageNameIndexTerm = (ValuePackageNameIndexTerm) term;
            }
        }

        final SearchEmptyQueryBuilder queryBuilder = new SearchEmptyQueryBuilder()
                .addTerm(sharedPartIndexTerm);

        if (moduleRootPathIndexTerm != null) {
            queryBuilder.addTerm(moduleRootPathIndexTerm);
        }
        if (packageNameIndexTerm != null) {
            queryBuilder.addTerm(packageNameIndexTerm);
        }
    ...
    public void validateTerms(Set<ValueIndexTerm> queryTerms) throws IllegalArgumentException {
        ...
        checkInvalidAndRequiredTerms(queryTerms,
                                     ...
                                     (t) -> (t.getTerm().equals(scorecardTerm.getTerm())));
    }
